...
You are an expert problem designer. Given the following hard math/reasoning \
problem, generate {batch_size} new problems that are **similar in structure and \
topic** but differ in specific values, constraints, or context.
Requirements for each generated problem:
- The answer must be a single numerical value (an integer, fraction, or decimal). \
  Do NOT create problems whose answer is a function, proof, set, or explanation.
- Should test the same core reasoning skill as the original.
- the problems should be challenging enough that a strong LLM gets them wrong 20-40 percent of the time
...
```json
[
  {{"problem": "Find all integers n > 1 such that ..."}},
...
def generate_similar_problems(
    ...
    Returns a list of dicts with keys "problem" and "difficulty_hint".
    """
    prompt = GENERATE_PROBLEMS_PROMPT.format(
        problem=hard_problem, batch_size=batch_size
    )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as pool:
        while len(dataset.problems) < n_target and round_num < max_generation_rounds:
            ...
            candidates = generate_similar_problems(
                client, model, hard_problem, batch_size=batch_size
            )
